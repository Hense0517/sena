...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="inventario_ general_consulta" && $field=="SUM(Cantidad)")
	{
		;
		return $value;
	}
				if($table=="Retiros" && $field=="Puerto")
	{
		;
		return $value;
	}
				if($table=="Inventario_Contratistas" && $field=="SUM(Cantidad)")
	{
		if ($value < 5) {  
    $color="red";  
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="Instalaciones" && $field=="Tecnico")
	{
		$strWhere = $_SESSION;
		return $strWhere;
	}
		if($table=="Mantenimientos" && $field=="Tecnico")
	{
		$strWhere = $_SESSION;
		return $strWhere;
	}
		if($table=="Retiros" && $field=="Tecnico")
	{
		$strWhere = $_SESSION;
		return $strWhere;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="Informe de Tickets" && $field=="Observaciones")
	{
		return Ninguna;
	}
				if($table=="logistica inversa" && $field=="Entregado en Bodega")
	{
		return No;
	}
				if($table=="Instalaciones" && $field=="Empresa")
	{
		return $_SESSION['id_empresa'];
	}
				if($table=="Instalaciones" && $field=="Tipo de Actividad")
	{
		return Instalación;
	}
				if($table=="Mantenimientos" && $field=="Empresa")
	{
		return $_SESSION['id_empresa'];
	}
				if($table=="Mantenimientos" && $field=="Tipo de Actividad")
	{
		return Mantenimiento;
	}
				if($table=="Retiros" && $field=="Empresa")
	{
		return $_SESSION['id_empresa'];
	}
				if($table=="Retiros" && $field=="Tipo de Actividad")
	{
		return Retiro;
	}
				if($table=="Retiros" && $field=="Puerto")
	{
		return 0;
				...
				if($table=="Retiros" && $field=="Cto")
	{
		return 0;
	}
				if($table=="Retiros" && $field=="Spliter")
	{
		return 0;
	}
				if($table=="Retiros" && $field=="Cable")
	{
		return 0;
	}
				if($table=="Material Reintegrado a Bodega" && $field=="Cargue o Descargue")
	{
		return Devolución;
	}
				if($table=="Cargue de Material a Tecnicos" && $field=="Cargue o Descargue")
	{
		return Cargado;
	}
				if($table=="Cargue Material A Bodega Contratistas" && $field=="Cargue o Descargue")
	{
		return Cargado;
	}
				if($table=="Cargue de Material a Tecnicos Contratistas" && $field=="Cargue o Descargue")
	{
		return Cargado;
	}
	return "";
...
function GetAutoUpdateValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="mega_ya_tecnicos_audit" && $field=="datetime")
	{
		return strftime("%Y-%m-%d (%H-1):%M:%S");
	}
	return "";
}
